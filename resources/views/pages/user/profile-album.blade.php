...
@section('content')
    <div class="mt-5 h-auto w-[94vw] mx-auto flex gap-3 flex-wrap">
        ...
        @foreach ($album as $item)
            <div
                class="darken-brightness relative w-52 h-72 bg-gray-200 rounded-3xl flex flex-col items-center cursor-pointer hover:bg-white transition duration-100 shadow-lg overflow-hidden" id="{{ $item->id }}">
                <img src="{{ asset('storage/post/' . $thumbnail->file_location) }}" class="w-full h-full object-cover">
                <p
