...
@section('content')
    ...
    <div class="hidden absolute w-5 h-5 bottom-6 right-16 rounded-full z-40 hover:cursor-pointer no-darken" id="add">
        <p class="text-center w-8 h-8 bg-white rounded-full text-xl font-bold">+</p>
    </div>
@endsection
...
@section('scripts')
    <script>
        ...
        const urlPost = (param) => {
            ...
            // URL endpoint untuk POST request
            const postEndpoint = '/add-image-album';
            ...
            fetch(postEndpoint, requestOptions)
                ...
                .then(data => {
                    // console.log(data.url);
                    window.location.href = "/creation/" + data.url;
                })
                .catch(error => {
                    // Tangani kesalahan jika terjadi
                    console.error('Error:', error);
